...
│   ├── GenerateMixinConfig.java         # Build-time helper — emits predefined-classes-config.json
│   ├── controller/
...
1. **`prepare-package` phase** — `GenerateMixinConfig` runs via `exec-maven-plugin`.
   ...
   generated `$Mixin` class bytes, and writes:
   - `target/agent-extracted-predefined-classes/<sha256>.classdata`
   - `target/predefined-classes-config.json`
...
2. **`package` phase** — `native-maven-plugin` invokes `native-image` with:
   ...
   - `-H:PredefinedClassesConfigurationFiles=target/predefined-classes-config.json`

The output binary is `target/kotowari-graalvm-example`.

### Why predefined classes are needed

`MixinUtils.createFactory()` uses the Java Class File API to generate a concrete
`DefaultHttpRequest$Mixin` subclass at startup via `MethodHandles.Lookup.defineClass()`.
GraalVM forbids dynamic class definition at native-image runtime.
The predefined-classes mechanism embeds the pre-generated bytecode into the image so
that `defineClass()` succeeds at runtime by matching the SHA-256 of the bytes.
